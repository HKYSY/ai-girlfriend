...
### 表情包系统（v1.5）
- 用户上传表情包（GIF 冻结首帧预览，避免动态图闪烁）
- **AI 自动匹配**：根据对话情绪标签智能挑选合适表情发送
- 数据库持久化存储，支持分类 / 关键词 / 情绪标签
- **批量扫描导入**：一键扫描 `temp` 目录把素材批量入库
- 表情搜索、最近使用、批量勾选删除管理
- 发送表情触发 AI 流式回复（SSE），对话更生动
...
- **v1.4**：Live2D 切换优化 + UI 动画系统 + AI 回复 Bug 修复
- **v1.5**：表情包系统（用户上传 + AI 自动匹配 + 数据库存储）
- **v1.6**：表情包面板重构（毛玻璃 UI + 批量扫描导入 + 批量删除 + 最近使用 + 流式发送 + GIF 首帧冻结 + 超时保护）
